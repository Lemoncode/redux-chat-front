...
import { onMessageListReceived, onMessageReceived } from '../actions/actions';
import { ApiModel } from '../api';
import { establishRoomSocketConnection } from './business';
import { mapApiMessagesToMessages, mapApiSimpleMessageToMessage } from './mappers';
...
function subscribe(socket) {
  return eventChannel(emit => {
    ...
    socket.on('messages', (messageList: ApiModel.Message[]) => {
      emit(onMessageListReceived(mapApiMessagesToMessages(messageList)));
    });
